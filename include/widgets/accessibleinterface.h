...
#define FUNC_DOACTION(classname)                                  \
  ...
  {                                                               \
    if (actionName == pressAction()) {                            \
      ...
      QMouseEvent eventr(QEvent::MouseButtonRelease,              \
                         localPos,                                \
                         Qt::LeftButton,                          \
                         Qt::LeftButton,                          \
                         Qt::NoModifier);                         \
...
#define FUNC_CHILD_LABLE(classname)                                   \
  QAccessibleInterface *Accessible##classname::child(int index) const \
  {                                                                   \
    return QAccessibleWidget::child(index);                           \
  }

// EDITABLE控件特有功能
#define FUNC_DELETETEXT(classname)                                          \
  void Accessible##classname::deleteText(int startOffset, int endOffset)    \
  {                                                                         \
    m_w->setText(m_w->text().remove(startOffset, endOffset - startOffset)); \
  }

#define FUNC_INSERTTEXT(classname)                                             \
  void Accessible##classname::insertText(int startOffset, const QString &text) \
  {                                                                            \
    m_w->setText(m_w->text().insert(startOffset, text));                       \
  }

#define FUNC_REPLACETEXT(classname)                                                            \
  void Accessible##classname::replaceText(int startOffset, int endOffset, const QString &text) \
  {                                                                                            \
    m_w->setText(m_w->text().replace(startOffset, endOffset - startOffset, text));             \
  }

#define FUNC_CHILD_EDITABLE(classname)                                \
  QAccessibleInterface *Accessible##classname::child(int index) const \
  {                                                                   \
    return nullptr;                                                   \
  }

#define FUNC_SELECTION(classname)                                                             \
  void Accessible##classname::selection(int selectionIndex, int *startOffset, int *endOffset) \
          const                                                                               \
  {                                                                                           \
...
#define FUNC_SELECTIONCOUNT(classname)              \
  int Accessible##classname::selectionCount() const \
  {                                                 \
    return m_w->hasSelectedText() ? 1 : 0;          \
  }

#define FUNC_ADDSELECTION(classname)                                       \
  void Accessible##classname::addSelection(int startOffset, int endOffset) \
  {                                                                        \
    setSelection(0, startOffset, endOffset);                               \
  }

#define FUNC_REMOVESELECTION(classname)                           \
  void Accessible##classname::removeSelection(int selectionIndex) \
  {                                                               \
    if (selectionIndex != 0)                                      \
      return;                                                     \
    m_w->deselect();                                              \
...
#define FUNC_SETSELECTION(classname)                                                           \
  void Accessible##classname::setSelection(int selectionIndex, int startOffset, int endOffset) \
  {                                                                                            \
    if (selectionIndex != 0)                                                                   \
      return;                                                                                  \
...
#define FUNC_CURORPOSITION(classname)               \
  int Accessible##classname::cursorPosition() const \
  {                                                 \
    return m_w->cursorPosition();                   \
  }

#define FUNC_SETCURORPOSITION(classname)                      \
  void Accessible##classname::setCursorPosition(int position) \
  {                                                           \
    m_w->setCursorPosition(position);                         \
  }

#define FUNC_TEXT_LineEdit(classname)                                       \
  QString Accessible##classname::text(int startOffset, int endOffset) const \
  {                                                                         \
...
#define FUNC_TEXTBEFOREOFFSET(classname)                                                      \
  QString Accessible##classname::textBeforeOffset(int offset,                                 \
  ...
  {                                                                                           \
    ...
    return QAccessibleTextInterface::textBeforeOffset(offset,                                 \
                                                      boundaryType,                           \
                                                      startOffset,                            \
                                                      endOffset);                             \
  }

#define FUNC_TEXTAFTEROFFSET(classname)                                                      \
  QString Accessible##classname::textAfterOffset(int offset,                                 \
                                                 QAccessible::TextBoundaryType boundaryType, \
                                                 int *startOffset,                           \
                                                 int *endOffset) const                       \
  {                                                                                          \
    ...
    if (offset == -2)                                                                        \
      offset = cursorPosition();                                                             \
    return QAccessibleTextInterface::textAfterOffset(offset,                                 \
                                                     boundaryType,                           \
                                                     startOffset,                            \
                                                     endOffset);                             \
  }

#define FUNC_TEXTATOFFSET(classname)                                                             \
  QString Accessible##classname::textAtOffset(int offset,                                        \
                                              QAccessible::TextBoundaryType boundaryType,        \
                                              int *startOffset,                                  \
                                              int *endOffset) const                              \
  {                                                                                              \
...
#define FUNC_CHARACTERCOUNT(classname)              \
  int Accessible##classname::characterCount() const \
  {                                                 \
    return m_w->text().count();                     \
  }

#define FUNC_CHARACTERRECT(classname)                          \
  QRect Accessible##classname::characterRect(int offset) const \
  {                                                            \
    return QRect();                                            \
  }

#define FUNC_OFFSETATPOINT(classname)                                 \
  int Accessible##classname::offsetAtPoint(const QPoint &point) const \
  {                                                                   \
    QPoint p = m_w->mapFromGlobal(point);                             \
    return m_w->cursorPositionAt(p);                                  \
  }

#define FUNC_SCROLLTOSUBSTRING(classname)                                     \
  void Accessible##classname::scrollToSubstring(int startIndex, int endIndex) \
  {                                                                           \
    m_w->setCursorPosition(endIndex);                                         \
    m_w->setCursorPosition(startIndex);                                       \
  }

#define FUNC_ATTRIBUTES(classname)                                                              \
  QString Accessible##classname::attributes(int offset, int *startOffset, int *endOffset) const \
  {                                                                                             \
...
#define SET_LABEL_ACCESSIBLE_WITH_DESCRIPTION(classname, accessiblename, type, accessdescription) \
  class Accessible##classname : public QAccessibleWidget, public QAccessibleTextInterface         \
  {                                                                                               \
public:                                                                                           \
...
#define SET_SLIDER_ACCESSIBLE_WITH_DESCRIPTION(classname, accessiblename, accessdescription) \
  class Accessible##classname : public QAccessibleWidget, public QAccessibleValueInterface   \
  {                                                                                          \
public:                                                                                      \
...
private:                                                                                          \
  ...
  FUNC_TEXT(classname, accessiblename)                                                            \
  FUNC_TEXT_LineEdit(classname) FUNC_DELETETEXT(classname) FUNC_INSERTTEXT(classname)             \
          FUNC_REPLACETEXT(classname) FUNC_SELECTION(classname) FUNC_SELECTIONCOUNT(classname)    \
                  FUNC_ADDSELECTION(classname) FUNC_REMOVESELECTION(classname)                    \
                          FUNC_SETSELECTION(classname) FUNC_CURORPOSITION(classname)              \
                                  FUNC_SETCURORPOSITION(classname) FUNC_CHARACTERCOUNT(classname) \
...
#define USE_ACCESSIBLE_BY_OBJECTNAME(classnamestring, classname, objectname) \
  if (classnamestring == QLatin1String(#classname) && object                 \
      && (object->objectName() == objectname) && object->isWidgetType()) {   \
    interface = new Accessible##classname(static_cast<classname *>(object)); \
  }
...
{
public:
    static AccessibleFactoryBase *RegisterAccessibleFactory(const char *factoryName,
                                                            AccessibleFactoryBase *factory);
};
...
public:                                                                                    \
  ...
  AccessibleFactoryBase *FactoryAccessible##ClassName::s_Accessible##ClassName##instance = \
          AccessibleFactoryManager::RegisterAccessibleFactory(Key,                         \
                                                              new FactoryAccessible##ClassName());

/////////////////////////////////////////////////
#define SET_FORM_ACCESSIBLE(classname, accessiblename)                \
  SET_FORM_ACCESSIBLE_WITH_DESCRIPTION(classname, accessiblename, "") \
  FactoryMacro(#classname, classname)

#define SET_BUTTON_ACCESSIBLE(classname, accessiblename)                \
  SET_BUTTON_ACCESSIBLE_WITH_DESCRIPTION(classname, accessiblename, "") \
  FactoryMacro(#classname, classname)

#define SET_LABEL_ACCESSIBLE(classname, accessiblename)                                         \
  SET_LABEL_ACCESSIBLE_WITH_DESCRIPTION(classname, accessiblename, QAccessible::StaticText, "") \
  FUNC_CHILD_LABLE(classname)                                                                   \
  FactoryMacro(#classname, classname)

#define SET_DTK_EDITABLE_ACCESSIBLE(classname, accessiblename)                                    \
  SET_LABEL_ACCESSIBLE_WITH_DESCRIPTION(classname, accessiblename, QAccessible::EditableText, "") \
  FUNC_CHILD_EDITABLE(classname)                                                                  \
  FactoryMacro(#classname, classname)

#define SET_SLIDER_ACCESSIBLE(classname, accessiblename)                \
  SET_SLIDER_ACCESSIBLE_WITH_DESCRIPTION(classname, accessiblename, "") \
  FactoryMacro(#classname, classname)

#define SET_EDITABLE_ACCESSIBLE(classname, accessiblename)                \
  SET_EDITABLE_ACCESSIBLE_WITH_DESCRIPTION(classname, accessiblename, "") \
  FactoryMacro(#classname, classname)
/////////////////////////////////////////////////
