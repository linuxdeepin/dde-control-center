...
void UpdateWorker::init()
{
    ...
    connect(m_updateInter,
            &UpdateDBusProxy::AutoCleanChanged,
            m_model,
            &UpdateModel::setAutoCleanCache);

    connect(m_updateInter,
            &UpdateDBusProxy::AutoDownloadUpdatesChanged,
            m_model,
            &UpdateModel::setAutoDownloadUpdates);
    connect(m_updateInter,
            &UpdateDBusProxy::AutoInstallUpdatesChanged,
            m_model,
            &UpdateModel::setAutoInstallUpdates);
    connect(m_updateInter,
            &UpdateDBusProxy::AutoInstallUpdateTypeChanged,
            m_model,
            &UpdateModel::setAutoInstallUpdateType);
    connect(m_updateInter,
            &UpdateDBusProxy::AutoCheckUpdatesChanged,
            m_model,
    ...
    connect(m_updateInter,
            &UpdateDBusProxy::RunningChanged,
            m_model,
            &UpdateModel::setAtomicBackingUp);

    connect(m_updateInter,
            &UpdateDBusProxy::ClassifiedUpdatablePackagesChanged,
            this,
            &UpdateWorker::onClassifiedUpdatablePackagesChanged);
    connect(m_updateInter, &UpdateDBusProxy::OnBatteryChanged, this, &UpdateWorker::setOnBattery);
    connect(m_updateInter,
            &UpdateDBusProxy::BatteryPercentageChanged,
            this,
            &UpdateWorker::setBatteryPercentage);
    connect(m_updateInter,
            &UpdateDBusProxy::StateChanged,
            this,
            &UpdateWorker::handleAtomicStateChanged);
}
...
CheckUpdateJobRet UpdateWorker::createCheckUpdateJob(const QString &jobPath)
{
    ...
    qDebug() << " Get Job: " << ret.jobID << ret.jobDescription << m_checkUpdateJob->progress()
             << m_checkUpdateJob->status();
    return ret;
}
                                                   ...
                                                   const QString &value)
{
    ...
    if (value == "running" || value == "ready") {
        ...
        QPointer<UpdateJobDBusProxy> job = getDownloadJob(type);
        qDebug() << "onClassityDownloadStatusChanged ::" << type
                                                  ...
                                                  const QString &value)
{
    ...
    if (value == "ready") {
        ...
        qDebug() << "onClassityInstallStatusChanged ::" << type
                 << "job->description() :: " << job->description();
        m_model->setClassityUpdateJonError(type, analyzeJobErrorMessage(job->description()));
        m_model->setClassifyUpdateTypeStatus(type, UpdatesStatus::UpdateFailed);
        cleanLastoreJob(job);
                                            ...
                                            QString message)
{
    /*
        ...
        -9: 本作的仓库本不存在;
    */
    qDebug() << " Atomic State : " << state << "operate: " << operate << " version: " << version
             << " message: " << message;
    switch (state) {
