...
BluetoothDBusProxy::BluetoothDBusProxy(QObject *parent)
    : QObject(parent)
    , m_bluetoothInter(new DCCDBusInterface(BluetoothService,
                                            BluetoothPath,
                                            BluetoothInterface,
                                            QDBusConnection::sessionBus(),
                                            this))
    , m_airPlaneModeInter(new DCCDBusInterface(AirPlaneModeService,
                                               AirPlaneModePath,
                                               AirPlaneModeInterface,
...
void BluetoothDBusProxy::showBluetoothTransDialog(const QString &address, const QStringList &files)
{
    QDBusMessage message =
            QDBusMessage::createMethodCall("com.deepin.filemanager.filedialog",
                                           "/com/deepin/filemanager/filedialogmanager",
                                           "com.deepin.filemanager.filedialogmanager",
                                           "showBluetoothTransDialog");
...
void BluetoothDBusProxy::ClearUnpairedDevice(QObject *receiver, const char *member)
{
    ...
    m_bluetoothInter->callWithCallback(QStringLiteral("ClearUnpairedDevice"),
                                       argumentList,
                                       receiver,
                                       member);
}
...
{
    m_bluetoothInter->asyncCall(QStringLiteral("SetAdapterPowered"),
                                QVariant::fromValue(adapter),
                                QVariant::fromValue(powered));
}

void BluetoothDBusProxy::SetAdapterPowered(const QDBusObjectPath &adapter,
                                           bool powered,
                                           QObject *receiver,
                                           const char *member,
                                           const char *errorSlot)
{
    ...
    m_bluetoothInter->callWithCallback(QStringLiteral("SetAdapterPowered"),
                                       argumentList,
                                       receiver,
                                       member,
                                       errorSlot);
...
{
    m_bluetoothInter->asyncCall(QStringLiteral("RemoveDevice"),
                                QVariant::fromValue(adapter),
                                QVariant::fromValue(device));
}

void BluetoothDBusProxy::ConnectDevice(const QDBusObjectPath &device,
                                       const QDBusObjectPath &adapter)
{
    m_bluetoothInter->asyncCall(QStringLiteral("ConnectDevice"),
                                QVariant::fromValue(device),
                                QVariant::fromValue(adapter));
}
...
{
    return QDBusPendingReply<QString>(m_bluetoothInter->asyncCall(QStringLiteral("GetDevices"),
                                                                  QVariant::fromValue(adapter)));
}

void BluetoothDBusProxy::GetDevices(const QDBusObjectPath &adapter,
                                    QObject *receiver,
                                    const char *member)
{
    QList<QVariant> argumentList;
    argumentList << QVariant::fromValue(adapter);
    m_bluetoothInter->callWithCallback(QStringLiteral("GetDevices"),
                                       argumentList,
                                       receiver,
                                       member);
}
...
{
    m_bluetoothInter->asyncCall(QStringLiteral("SetAdapterAlias"),
                                QVariant::fromValue(adapter),
                                QVariant::fromValue(alias));
}

void BluetoothDBusProxy::SetDeviceAlias(const QDBusObjectPath &device, const QString &alias)
{
    m_bluetoothInter->asyncCall(QStringLiteral("SetDeviceAlias"),
                                QVariant::fromValue(device),
                                QVariant::fromValue(alias));
}
...
{
    m_bluetoothInter->asyncCall(QStringLiteral("Confirm"),
                                QVariant::fromValue(device),
                                QVariant::fromValue(accept));
}

void BluetoothDBusProxy::SetAdapterDiscovering(const QDBusObjectPath &adapter, bool discovering)
{
    m_bluetoothInter->asyncCall(QStringLiteral("SetAdapterDiscovering"),
                                QVariant::fromValue(adapter),
                                QVariant::fromValue(discovering));
}

void BluetoothDBusProxy::SetAdapterDiscoverable(const QDBusObjectPath &adapter, bool discoverable)
{
    m_bluetoothInter->asyncCall(QStringLiteral("SetAdapterDiscoverable"),
                                QVariant::fromValue(adapter),
                                QVariant::fromValue(discoverable));
}
