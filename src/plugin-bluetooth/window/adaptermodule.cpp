...
AdapterModule::AdapterModule(const BluetoothAdapter *adapter,
...
{
    m_moduleList.append(
            new WidgetModule<SettingsGroup>("bluetoothTitleGroup",
                                            tr("Allow other Bluetooth devices to find this device"),
                                            this,
                                            &AdapterModule::initBluetoothTitle));
    m_bluetoothTitle = new WidgetModule<QLabel>(
            "bluetoothTitle",
            tr("Enable Bluetooth to find nearby devices (speakers, keyboard, mouse)"),
            [](QLabel *tip) {
                tip->setText(
                        tr("Enable Bluetooth to find nearby devices (speakers, keyboard, mouse)"));
    ...
    m_devicesTitle =
            new WidgetModule<TitleLabel>("devicesTitle", tr("My Devices"), [](TitleLabel *title) {
                title->setText(tr("My Devices"));
                DFontSizeManager::instance()->bind(title,
                                                   DFontSizeManager::T5,
                                                   QFont::DemiBold); // 设置字体
            });
    m_moduleList.append(m_devicesTitle);
    m_mydevicelist = new WidgetModule<DCCListView>("mydeviceList",
                                                   tr("My Devices"),
                                                   this,
                                                   &AdapterModule::initDeviceListView);
    m_moduleList.append(m_mydevicelist);
    m_otherDevices = new WidgetModule<TitleLabel>("otherDevices",
                                                  tr("Other Devices"),
                                                  [](TitleLabel *title) {
                                                      title->setText(tr("Other Devices"));
                                                      DFontSizeManager::instance()->bind(
                                                              title,
                                                              DFontSizeManager::T5,
                                                              QFont::DemiBold); // 设置字体
                                                  });
    m_moduleList.append(m_otherDevices);
    m_anonymousCheckBox = new WidgetModule<QWidget>("anonymousCheckBox",
                                                    tr("Other Devices"),
                                                    this,
                                                    &AdapterModule::initAnonymousCheckBox);
    m_moduleList.append(m_anonymousCheckBox);
    m_otherdevicelist = new WidgetModule<DCCListView>("otherdeviceList",
                                                      "",
                                                      this,
                                                      &AdapterModule::initOtherDeviceListView);
...
void AdapterModule::initBluetoothTitle(DCC_NAMESPACE::SettingsGroup *settingsGrp)
{
    ...
    SwitchWidget *discoverySwitch =
            new SwitchWidget(tr("Allow other Bluetooth devices to find this device"));
    ...
    connect(powerSwitch,
            ...
            this,
            [this, powerSwitch, discoverySwitch](const bool check) {
                // 关闭蓝牙的时候,直接隐藏列表
                if (!check) {
    ...
    connect(discoverySwitch,
            &SwitchWidget::checkedChanged,
            this,
            &AdapterModule::toggleDiscoverableSwitch);
    connect(m_adapter,
            &BluetoothAdapter::discoverableChanged,
            discoverySwitch,
            [this, discoverySwitch] {
                discoverySwitch->setChecked(m_adapter->discoverabled());
            });
    connect(m_adapter,
            &BluetoothAdapter::poweredChanged,
            powerSwitch,
            [powerSwitch, discoverySwitch](bool bPower, bool) {
...
void AdapterModule::initDeviceListView(DCCListView *deviceListView)
{
    ...
    connect(model,
            &BluetoothDeviceModel::requestSetDevAlias,
            this,
            &AdapterModule::requestSetDevAlias);
    connect(model,
            &BluetoothDeviceModel::detailClick,
            this,
            [this, deviceListView](const BluetoothDevice *device) {
...
void AdapterModule::initAnonymousCheckBox(QWidget *w)
{
    ...
    connect(m_model,
            &BluetoothModel::displaySwitchChanged,
    ...
    connect(m_adapter,
            &BluetoothAdapter::poweredChanged,
            spinner,
            [spinner, refreshBtn](bool bPower, bool bDiscovering) {
...
void AdapterModule::initOtherDeviceListView(DCCListView *otherDeviceListView)
{
    ...
    otherDeviceListView->setBackgroundType(
    ...
    connect(m_model,
            &BluetoothModel::displaySwitchChanged,
            model,
            &BluetoothDeviceModel::showAnonymous);
...
void AdapterModule::setAdapter(const BluetoothAdapter *adapter)
{
    connect(adapter,
            &BluetoothAdapter::poweredChanged,
            this,
            &AdapterModule::updateVisible,
            Qt::QueuedConnection);
    connect(adapter,
            &BluetoothAdapter::deviceAdded,
            this,
            &AdapterModule::deviceChanged,
            Qt::QueuedConnection);
    connect(adapter,
...
void AdapterModule::contextMenu(const BluetoothAdapter *adapter,
