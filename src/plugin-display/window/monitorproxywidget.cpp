...
void MonitorProxyWidget::paint(QPainter *painter,
                               ...
                               QWidget *widget)
{
    ...
    if (m_model->displayMode() != MERGE_MODE) {
        if (width > this->w()) {
            ...
            painter->drawText(QRectF(r.x() + r.width() - width, r.y() + height, width, height),
                              Qt::AlignRight,
                              elidedText);
        } else {
            painter->drawText(
                    QRectF(r.x() + r.width() - width - height / 2, r.y() + height, width, height),
                    Qt::AlignCenter,
                    name());
        }
    }
    ...
    if (m_model->displayMode() == EXTEND_MODE) {
        ...
        if (m_monitor->isPrimary()) {
            ...
            painter->drawRoundedRect(
                    QRectF(r.x() + r.width() / 4.0, r.y() + r.height() * 9.0 / 10.0, width, height),
                    radius,
                    radius);
        }
