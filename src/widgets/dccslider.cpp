...
void DCCSlider::paintEvent(QPaintEvent *e)
{
    ...
    qreal percentage =
            (m_separateValue - slider->minimum()) * 1.0 / (slider->maximum() - slider->minimum());
    ...
    if (orient == Qt::Horizontal) {
        ...
        if (tick == QSlider::TicksAbove || tick == QSlider::TicksBelow
            || tick == QSlider::NoTicks) {
            pa.drawLine(QPointF(startX + sliderX, startY), QPointF(startX + sliderX, startY + 3));
            pa.drawLine(QPointF(startX + sliderX, startY + 9),
                        QPointF(startX + sliderX, startY + 12));
        }
    }
}
