...
void FingerWidget::setStatueMsg(const QString &title, const QString &msg, bool reset)
{
    ...
    if (!m_reset) {
        ...
        if (m_pro == 0) {
            m_view->setPictureSequence(QStringList() << QString(":/authentication/themes/%1/icons/finger/fingerprint_light.svg").arg(m_theme));
        } else {
            m_view->setPictureSequence(QStringList() << QString(":/authentication/themes/%1/icons/finger/fingerprint_animation_light_%2.svg")
