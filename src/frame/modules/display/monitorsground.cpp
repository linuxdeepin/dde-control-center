...
void MonitorsGround::initMonitorProxyWidget(Monitor *mon) {
    ...
    connect(pw, &MonitorProxyWidget::requestMonitorPress, this, [this, pw]() { this->m_movingItem = pw; });
...
void MonitorsGround::adjustAll() {
    ...
    int cnt = 0;
    int offset = 100;
    const double offsetX = (width() - m_model->screenWidth()) / 2;
    const double offsetY = (height() - m_model->screenHeight()) / 2;
    for (auto pw : m_monitors.keys()) {
        const double w = pw->w() / 2;
        const double h = pw->h() / 2;
        const double x = pw->x();
        const double y = pw->y();
...
void MonitorsGround::onResize() {
    if (!m_isSingleDisplay) {
        if (this->size().width() < MAX_W) {
            setSceneRect(QRect(0, 0, this->width(), 240 + this->size().width() - MIN_W));
            this->setFixedHeight(240 + this->size().width() - MIN_W);
        } else {
            setSceneRect(QRect(0, 0, this->width(), 240 + MAX_W - MIN_W));
            this->setFixedHeight(240 + MAX_W - MIN_W);
        }
    }
    resetMonitorsView();
}
...
void MonitorsGround::singleScreenAdjust() {
    auto *mon = m_monitors.lastKey();
    auto w = mon->w();
    auto h = mon->h();

    mon->setPos(0, 0);
    mon->setEdge(QPointF(w, h));
...
void MonitorsGround::applySettings() {
    ...
    std::sort(lstX.begin(), lstX.end(), [=](const qreal A, const qreal B) { return A < B; });
    std::sort(lstY.begin(), lstY.end(), [=](const qreal A, const qreal B) { return A < B; });
    ...
    for (auto it(m_monitors.cbegin()); it != m_monitors.cend(); ++it) {
        ...
        qWarning() << "applySettings" << it.value()->name() << int(it.key()->pos().x() - lstX.first())
                   << int(it.key()->pos().y() - lstY.first()) << it.key()->pos();
    }
...
QPointF MonitorsGround::multiScreenSortAlgo(bool &isRestore, bool isMove) {
    ...
    QRectF moveItemTemp, moveItemIntersect;
    ...
    for (int i = 0; i < m_lstSortItems.size(); i++) {
        ...
        if (m_movingItem != m_lstSortItems[i]) {
            moveItemTemp = moveItemTemp |
                           m_movingItem->mapRectToScene(m_movingItem->justIntersectRect())
                               .intersected(m_lstSortItems[i]->mapRectToScene(m_lstSortItems[i]->boundingRect()));

            //移动块完全覆盖一个块
            if (m_movingItem->mapRectToScene(m_movingItem->justIntersectRect())
                    .contains(m_lstSortItems[i]->mapRectToScene(m_lstSortItems[i]->boundingRect()))) {
                lstShelterItems.append(m_lstSortItems[i]);
            }

            //要么不相交,要相交就是要点线相交的
            if (!m_movingItem->mapToScene(m_movingItem->justIntersectRect())
                     .intersects(m_lstSortItems[i]->mapToScene(m_lstSortItems[i]->boundingRect())) &&
                m_movingItem->mapToScene(m_movingItem->boundingRect())
                    .intersects(m_lstSortItems[i]->mapToScene(m_lstSortItems[i]->boundingRect()))) {
    ...
    //移动块被完全包含在其他的块
    if ((moveItemIntersect | moveItemTemp) == moveItemTemp) {
        qDebug() << "存在包含关系";
    ...
    std::sort(m_lstMoveingItemToCenterPosLen.begin(),
              ...
              [=](const QPair<MonitorProxyWidget *, qreal> &A, const QPair<MonitorProxyWidget *, qreal> &B) {
                  return A.second < B.second;
              });

    // 1、没有重合的块或者一个重合块的处理方式
    if (lstShelterItems.size() > 0) {
        ...
        std::sort(lstNoMovedItems.begin(),
                  lstNoMovedItems.end(),
                  [=](const MonitorProxyWidget *A, const MonitorProxyWidget *B) {
                      return A->mapToScene(A->boundingRect().bottomLeft()).x() <
                             B->mapToScene(B->boundingRect().bottomLeft()).x();
                  });
        ...
        std::sort(lstNoMovedItems.begin(),
                  ...
                  [=](const MonitorProxyWidget *A, const MonitorProxyWidget *B) {
                      if (A == m_movingItem) {
                          return A->mapToScene(A->boundingRect().bottomRight()).x() <
                                 B->mapToScene(B->boundingRect().bottomLeft()).x();
                      } else if (B == m_movingItem) {
                          return A->mapToScene(A->boundingRect().bottomLeft()).x() <
                                 B->mapToScene(B->boundingRect().bottomRight()).x();
                      }
                      return A->mapToScene(A->boundingRect().bottomLeft()).x() <
                             B->mapToScene(B->boundingRect().bottomLeft()).x();
                  });
        ...
        std::sort(lstNoMovedItems.begin(),
                  ...
                  [=](const MonitorProxyWidget *A, const MonitorProxyWidget *B) {
                      if (A == m_movingItem) {
                          return A->mapToScene(A->boundingRect().bottomLeft()).y() <
                                 B->mapToScene(B->boundingRect().topLeft()).y();
                      } else if (B == m_movingItem) {
                          return A->mapToScene(A->boundingRect().topLeft()).y() <
                                 B->mapToScene(B->boundingRect().bottomLeft()).y();
                      }
                      return A->mapToScene(A->boundingRect().topLeft()).y() <
                             B->mapToScene(B->boundingRect().topLeft()).y();
                  });
        ...
        if (!(nBIndexLeft == 0 || nBIndexRight == nItemSize)) {
            ...
            if (nIndexTop == nIndexBottom) {
                if (nIndexTop == 0) {
                    qreal dy = lstActivedItemsX.first()
                                   ->mapToScene(lstActivedItemsX.first()->boundingRect().topLeft())
                                   .y() -
                               m_movingItem->mapToScene(m_movingItem->boundingRect().bottomLeft()).y();
                    m_movingItem->executeMoveBy(0, dy);
                    ...
                    for (int i = 0; i < lstNoMovedItems.size(); i++) {
                        if (m_movingItem->mapToScene(m_movingItem->boundingRect())
                                .intersects(lstNoMovedItems[i]->mapToScene(lstNoMovedItems[i]->boundingRect()))) {
                            lstShelterItemsTemp.append(lstNoMovedItems[i]);
                        }
                    }

                    if (lstShelterItemsTemp.size() >= 0) {
                        m_movingItem->executeMoveBy(0, -dy); //先移回去
                        m_graphicsScene.update();
                        ...
                        nIndexBottom = lstActivedItemsX.indexOf(m_movingItem);

                        //上排序
                        for (auto t : lstShelterItemsTemp) {
                            if (!lstActivedItemsX.contains(t)) lstActivedItemsX.append(t);
                        }
                        std::sort(lstActivedItemsX.begin(),
                                  lstActivedItemsX.end(),
                                  [=](const MonitorProxyWidget *A, const MonitorProxyWidget *B) {
                        ...
                        nIndexTop = lstActivedItemsX.indexOf(m_movingItem);
                        ...
                        if (!lstActivedItemsX.isEmpty()) {
                            if (nIndexTop == 0) {
                                ...
                                g_dy = lstActivedItemsX.last()
                                           ->mapToScene(lstActivedItemsX.last()->boundingRect().bottomLeft())
                                           .y() -
                    ...
                    // TODO：move to
                    qreal dy = lstActivedItemsX[0]->mapToScene(lstActivedItemsX[0]->boundingRect().bottomLeft()).y() -
                               m_movingItem->mapToScene(m_movingItem->boundingRect().topLeft()).y();
                    m_movingItem->executeMoveBy(0, dy);
                    m_graphicsScene.update();
        ...
        if (!(nBIndexTop == 0 || nBIndexBottom == nItemSize)) {
            ...
            std::sort(lstActivedItemsY.begin(),
                      ...
                      [=](const MonitorProxyWidget *A, const MonitorProxyWidget *B) {
                          return A->mapToScene(A->boundingRect().topLeft()).x() <
                                 B->mapToScene(B->boundingRect().topLeft()).x();
                      });
            ...
            if (nIndexLeft == nIndexRight) {
                if (nIndexLeft == 0) {
                    ...
                    if (lstShelterItemsTemp.size() >= 0) {
                        ...
                        nIndexRight = lstActivedItemsY.indexOf(m_movingItem);

                        //上排序
                        for (auto t : lstShelterItemsTemp) {
                            if (!lstActivedItemsY.contains(t)) lstActivedItemsY.append(t);
                        }
                        std::sort(lstActivedItemsY.begin(),
                                  lstActivedItemsY.end(),
                                  [=](const MonitorProxyWidget *A, const MonitorProxyWidget *B) {
                        ...
                        nIndexLeft = lstActivedItemsY.indexOf(m_movingItem);
                    ...
                    qreal dx = lstActivedItemsY.last()
                                   ->mapToScene(lstActivedItemsY.last()->boundingRect().topRight())
                                   .x() -
                               m_movingItem->mapToScene(m_movingItem->boundingRect().topLeft()).x();
                    m_movingItem->executeMoveBy(dx, 0);
        ...
        if ((nBIndexLeft == 0 || nBIndexRight == nItemSize) || (nBIndexTop == 0 || nBIndexBottom == nItemSize)) {
            ...
            if (nBIndexLeft == 0 && nBIndexTop == 0) {
                ...
                std::sort(lstTempLen.begin(),
                          lstTempLen.end(),
                          [=](const QPair<MonitorProxyWidget *, qreal> &A,
                              const QPair<MonitorProxyWidget *, qreal> &B) { return A.second < B.second; });
    ...
    if (isAutoAdsorption && !isIntersect) {
        m_movingItem->executeMoveBy(0, 0);
    } else {
        if (g_bXYTogetherMoved) {
            m_movingItem->executeMoveBy(g_dx, g_dy);
            ...
            if (qFuzzyIsNull(g_dx) || qFuzzyIsNull(g_dy)) {
                ...
                fabs(g_dx) < fabs(g_dy) ? m_movingItem->executeMoveBy(g_dx, 0) : m_movingItem->executeMoveBy(0, g_dy);
...
void MonitorsGround::multiScreenAutoAdjust() {
    ...
    //变化的块只存在移动屏幕则返回
    if (lstChangedItems.contains(m_movingItemTemp) && lstChangedItems.size() == 1) {
        return;
    }
    ...
    for (auto iter : m_lstItems) {
        if (!lstChangedItems.contains(iter)) continue;
    ...
    if (maplstItems.size() > 0) {
        ...
        for (auto iter = maplstItems.begin(); iter != maplstItems.end(); ++iter) {
            if (iter.value().contains(m_movingItemTemp)) continue;
            ...
            if (!lstPos.isEmpty()) {
                //排序，根据绝对值排序
                std::sort(lstPos.begin(), lstPos.end(), [=](QPointF A, QPointF B) {
                    return fabs(A.x()) < fabs(B.x());
                });

                mindX = lstPos.first().x();

                //                std::sort(lstPos.begin(),lstPos.end(),[=](QPointF A,  QPointF B) {
                //                   return fabs(A.y()) < fabs(B.y());
                //                });
                mindY = lstPos.first().y();
            }

            for (auto item : iter.value()) {
                m_movingItem = item;
                m_movingItem->executeMoveBy(mindX, mindY);
                m_graphicsScene.update();
                if (!m_lstSortItems.contains(item)) m_lstSortItems.append(item);
            }
        }
    }
...
void MonitorsGround::updateConnectedState(bool isInit) {
    ...
    for (int i = 0; i < m_lstItems.size(); i++) {
        ...
        for (int j = 0; j < m_lstItems.size(); j++) {
            if (j != i &&
                m_lstItems[i]
                    ->mapToScene(m_lstItems[i]->boundingRectEx())
                    .intersects(m_lstItems[j]->mapToScene(m_lstItems[j]->boundingRect())) &&
                !m_lstItemsTemp.contains(m_lstItems[j]) &&
                !m_lstItems[i]
                     ->mapToScene(m_lstItems[i]->justIntersectRect())
                     .intersects(m_lstItems[j]->mapToScene(m_lstItems[j]->boundingRect()))) {
    ...
    for (auto item : m_lstItems) {
        ...
        if (pw->mapToScene(pw->boundingRect()).intersects(item->mapToScene(item->boundingRect()))) {
            if (item->mapToScene(item->boundingRect()).intersects(pw->mapToScene(pw->justIntersectRectTop()))) {
                //上相交
                bIntersectsTop = true;
            } else if (item->mapToScene(item->boundingRect())
                           .intersects(pw->mapToScene(pw->justIntersectRectBottom()))) {
                //下相交
                ...
                bIntersectsLeft = true;
                           ...
                           .intersects(pw->mapToScene(pw->justIntersectRectRight()))) {
                //右相交
        ...
        for (auto item : m_lstItems) {
            ...
            if (item->mapToScene(item->boundingRect()).intersects(pw->mapToScene(pw->justIntersectRectLeftTop()))) {
                ...
                bIntersectsRight = true;
                           ...
                           .intersects(pw->mapToScene(pw->justIntersectRectLeftBottom()))) {
                //左下相交
                bIntersectsTop = true;
                bIntersectsRight = true;
            } else if (item->mapToScene(item->boundingRect())
                           .intersects(pw->mapToScene(pw->justIntersectRectRightTop()))) {
                //右上相交
                bIntersectsBottom = true;
                bIntersectsLeft = true;
            } else if (item->mapToScene(item->boundingRect())
                           .intersects(pw->mapToScene(pw->justIntersectRectRightBottom()))) {
                //右下相交
                bIntersectsTop = true;
                bIntersectsLeft = true;
            }
...
void MonitorsGround::onRequestMouseMove(MonitorProxyWidget *pw) {
    ...
    for (auto item : m_lstItems) {
        ...
        if (pw->mapToScene(pw->boundingRect()).intersects(item->mapToScene(item->boundingRect()))) return;
...
double MonitorsGround::screenScale() const {
    const double scaleW = double(width()) / m_model->screenWidth();
    const double scaleH = double(height()) / m_model->screenHeight();

    return std::min(scaleW, scaleH);
}

void MonitorsGround::autoRebound() {
    QMap<MonitorProxyWidget *, QPointF *> mapPos;
    QPointF dPos;
    //获取偏移量
    QPointF offsetPos;

    autoCenterData(mapPos, dPos, offsetPos);

    QRectF rect = scene()->itemsBoundingRect();
    if (m_isInit) {
        offsetPos.setX((width() - rect.width()) / 2);
        offsetPos.setY((height() - rect.height()) / 2);
    }

    for (auto pw : m_monitors.keys()) {
        //移动位置
        pw->executeMoveBy((pw->getPreCenter() - (*mapPos[pw] - dPos + offsetPos)).x(),
                          (pw->getPreCenter() - (*mapPos[pw] - dPos + offsetPos)).y());
        //设置位置
        pw->setPos(pw->getPreCenter());
        //设置大小
        pw->setEdge(pw->getPreEdge());
    }
...
void MonitorsGround::autoFitItemsInView() {
    auto rect = scene()->itemsBoundingRect();
    auto wOffset = rect.width() / 10;
    auto hOffset = rect.height() / 10;
    fitInView(rect.adjusted(-wOffset, -hOffset, wOffset, hOffset), Qt::KeepAspectRatio);
}
...
void MonitorsGround::centeredMonitorsView() {
    if (m_monitors.size() == 1) {
        ...
        for (auto pw : m_monitors.keys()) {
            //设置位置
            pw->setPos(*mapPos[pw] - dPos + offsetPos);
            //设置大小
            pw->setEdge(QPointF(pw->w(), pw->h()));

            //保存上一次调整的数据,用与在不满足调整条件的时候自动恢复的功能.
            pw->setPreCenter(*mapPos[pw] - dPos + offsetPos);
            pw->setPreEdge(QPointF(pw->w(), pw->h()));
        }
...
//获取自动居中的数据源
void MonitorsGround::autoCenterData(QMap<MonitorProxyWidget *, QPointF *> &mapPos, QPointF &dPos, QPointF &offsetPos) {
    for (auto pw : m_monitors.keys()) {
        mapPos.insert(pw, new QPointF(pw->x() + pw->dx(), pw->y() + pw->dy()));
    }

    //处理位置坐标 根据位置信息向中心调整,取X最小值设置为0,取Y的最小值设置为0
    QList<QPointF *> lstPos = mapPos.values();

    //取X最小值
    std::sort(lstPos.begin(), lstPos.end(), [=](const QPointF *A, const QPointF *B) { return A->x() < B->x(); });
    dPos.setX(lstPos.first()->x());

    //取Y最小值
    std::sort(lstPos.begin(), lstPos.end(), [=](const QPointF *A, const QPointF *B) { return A->y() < B->y(); });
    dPos.setY(lstPos.first()->y());

    //获取偏移量
    offsetPos.setX((width() - m_model->screenWidth()) / 2);
    offsetPos.setY((height() - m_model->screenHeight()) / 2);
}
