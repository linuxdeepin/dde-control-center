...
public:
    ...
    static QSize drawActions(QPainter *pa, const QStyleOptionViewItem &option, const QVariant &value, Qt::Edge edge, QList<QPair<QAction*, QRect>> *clickableActionRect, int leftoffset = 0)
    {
        ...
        origin.setX(origin.x() + leftoffset);
...
void DStyledItemDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    int offset = 0;
    if (index.model()->objectName() == "cursor") {
        offset = 6;
    }
    action_area_size = d->drawActions(painter, opt, index.data(Dtk::BottomActionListRole), Qt::BottomEdge, &clickActionList, offset);
