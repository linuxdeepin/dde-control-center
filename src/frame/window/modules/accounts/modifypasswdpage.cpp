...
bool ModifyPasswdPage::onPasswordEditFinished(Dtk::Widget::DPasswordEdit *edit)
{
    ...
    case _ENUM_PASSWORD_SEVERAL:
        edit->setAlert(true);
        edit->showAlertMessage(tr("The password must have at least %1 characters, and contain at least %1 of the four available ").arg(m_passwordMinLength).arg(m_validateRequiredString), -1);
        return false;
    }
