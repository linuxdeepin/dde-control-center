...
void FaceWidget::onFaceidListChanged(const QStringList &facelist)
{
    ...
    // 找到最小的名称以便作为缺省名添加
    for (int i = 0; i < m_model->getPredefineFaceName().size(); ++i) {
        bool findNotUsedThumb = false;
        QString newFingerName = m_model->getPredefineFaceName().at(i);

        for (int n = 0; n < FACEID_NUM && n < facelist.size(); ++n) {
            if (newFingerName == facelist.at(n)) {
