...
TouchScreenModule::TouchScreenModule(QObject *parent)
    : PageModule("touchscreen",
                 tr("Touch Screen"),
                 tr("Touch Screen"),
                 QIcon::fromTheme("dcc_nav_touchscreen"),
                 parent)
{
...
void TouchScreenModule::init()
{
    ...
    appendChild(page);
    page->appendChild(new ItemModule(
            "touchScreenTipLabel",
            tr("Select your touch screen when connected or set it here."),
    ...
    connect(m_model,
            &TouchScreenModel::touchScreenListChanged,
            this,
            &TouchScreenModule::resetItems);
    connect(m_model, &TouchScreenModel::monitorsChanged, this, &TouchScreenModule::resetItems);
    resetItems();

    ItemModule *cancel = new ItemModule(
            "Cancel",
            "Cancel",
...
void TouchScreenModule::resetItems()
{
    ...
    for (const auto &touchScreen : m_model->touchScreenList()) {
        ItemModule *item = new ItemModule(
                "touchScreen",
                touchScreen.name,
                [monitors, touchScreen, touchScreenAndMonitor, this](
                        ModuleObject *module) -> QWidget * {
