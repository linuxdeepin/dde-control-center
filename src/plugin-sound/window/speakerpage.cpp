    ...
    , m_waitStatusChangeTimer(new QTimer(this))
{
    ...
    m_outputSoundsGrp =
            new DCC_NAMESPACE::SettingsGroup(nullptr,
                                             DCC_NAMESPACE::SettingsGroup::GroupBackground);
...
void SpeakerPage::setModel(SoundModel *model)
{
    ...
    connect(m_model, &SoundModel::isPortEnableChanged, this, [=](bool visible) {
        ...
        if (m_enablePort) {
            ...
            if (index.isValid()) {
                Q_EMIT requestSetPort(m_outputModel->data(index, Qt::WhatsThisPropertyRole)
                                              .value<const Port *>());
            }
        }
        showDevice();
    ...
    connect(m_model, &SoundModel::setPortChanged, this, [=](const Port *port) {
        ...
        Q_EMIT m_model->requestSwitchEnable(
                port->cardId(),
                port->id()); // 设置端口后，发送信号，判断该端口是否需要禁用
    });
    ...
    connect(m_outputSoundCbx->comboBox(),
            static_cast<void (QComboBox::*)(int)>(&QComboBox::activated),
            this,
            &SpeakerPage::changeComboxIndex,
            Qt::QueuedConnection);
    connect(m_model,
            &SoundModel::bluetoothModeOptsChanged,
            this,
            [=](const QStringList bluetoothModeOpts) {
    ...
    connect(m_model,
            ...
            this,
            &SpeakerPage::changeComboxStatus,
            Qt::UniqueConnection);
...
void SpeakerPage::removePort(const QString &portId,
                             const uint &cardId,
                             const Port::Direction &direction)
{
    ...
    for (int i = 0; i < m_outputModel->rowCount(); i++) {
        ...
        if (port && port->id() == portId && cardId == port->cardId()) {
            ...
            if (m_currentPort && m_currentPort->id() == portId
                && m_currentPort->cardId() == cardId) {
...
void SpeakerPage::changeComboxIndex(const int idx)
{
    ...
    auto tFunc = [this](const int tmpIdx) {
        ...
        this->requestSetPort(
                m_outputModel->data(temp, Qt::WhatsThisPropertyRole).value<const Port *>());
        qDebug() << "default sink index change, currentTerxt:"
                 << m_outputSoundCbx->comboBox()->itemText(tmpIdx);
    };
...
void SpeakerPage::initSlider()
{
    ...
    connect(m_model, &SoundModel::maxUIVolumeChanged, this, [=](double maxvalue) {
        ...
        m_speakSlider->blockSignals(false);
        m_outputSlider->setValueLiteral(
                QString::number((int)(m_model->speakerVolume() * 100 + 0.000001)) + "%");
    });
    ...
    // 下方提示
    auto volumeBoostTip = new DTipLabel(tr("If the volume is louder than 100%, it may distort "
                                           "audio and be harmful to output devices"),
                                        this);
...
void SpeakerPage::initCombox()
{
    ...
    connect(m_blueSoundCbx->comboBox(),
            static_cast<void (QComboBox::*)(int)>(&QComboBox::activated),
            this,
            &SpeakerPage::changeBluetoothMode);
...
{
    m_volumeBtn->setIcon(
            DStyle::standardIcon(style(),
                                 (m_model->speakerOn() ? DStyle::SP_MediaVolumeMutedElement
                                                       : DStyle::SP_MediaVolumeLowElement)));
}

void SpeakerPage::showWaitSoundPortStatus(bool showStatus)
{
    if ((m_currentPort && !m_currentPort->isBluetoothPort())
        || m_model->currentBluetoothAudioMode().isEmpty()) {
        m_blueSoundCbx->setVisible(false);
    }
