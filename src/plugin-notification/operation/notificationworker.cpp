    ...
    , m_dbus(new NotificationDBusProxy(this))
{
    ...
    connect(m_dbus,
            &NotificationDBusProxy::AppRemovedSignal,
            this,
            &NotificationWorker::onAppRemoved);
}
...
void NotificationWorker::initSystemSetting()
{
    ...
    item->setLockScreen(
            m_dbus->GetSystemInfo(SysItemModel::LOCKSCREENOPENDNDMODE).value().variant().toBool());
    item->setTimeSlot(
            m_dbus->GetSystemInfo(SysItemModel::OPENBYTIMEINTERVAL).value().variant().toBool());
    connect(m_dbus,
            &NotificationDBusProxy::SystemInfoChanged,
            item,
            &SysItemModel::onSettingChanged);
...
void NotificationWorker::onAppAdded(const QString &id)
{
    ...
    item->setAllowNotify(
            m_dbus->GetAppInfo(id, AppItemModel::ENABELNOTIFICATION).value().variant().toBool());
    item->setShowNotifyPreview(
            m_dbus->GetAppInfo(id, AppItemModel::ENABELPREVIEW).value().variant().toBool());
    item->setNotifySound(
            m_dbus->GetAppInfo(id, AppItemModel::ENABELSOUND).value().variant().toBool());
    item->setShowInNotifyCenter(m_dbus->GetAppInfo(id, AppItemModel::SHOWINNOTIFICATIONCENTER)
                                        .value()
                                        .variant()
                                        .toBool());
    item->setLockShowNotify(m_dbus->GetAppInfo(id, AppItemModel::LOCKSCREENSHOWNOTIFICATION)
                                    .value()
                                    .variant()
                                    .toBool());
