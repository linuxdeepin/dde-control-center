...
| ![deepin-multitasking-view](../common/deepin-multitasking-view.svg) | 多任务视图 - 点击显示工作区。       | ![dde-file-manager](../common/dde-file-manager.svg)      | 文件管理器 - 点击查看磁盘中的文件、文件夹。         |
| ![browser-cn](../common/browser-cn.svg)           | 浏览器 - 点击打开网页。             | ![deepin-appstore](../common/deepin-appstore.svg)        | 商店 - 搜索安装应用软件。                           |
...
| ![desktop-ai-assistant](../common/desktop-ai-assistant.svg)                               |   桌面智能助手 - 使用语音或文字来发布指令或进行询问。     |![onboard](../common/onboard.svg) | 屏幕键盘 - 点击使用虚拟键盘。 |
| ![shutdown](../common/system-shutdown.svg)                   | 电源 - 点击进入关机界面。            | ![trash](../common/trash_icon.svg)                       | 回收站。                                            |
...
智能助手支持语音和文字输入、查找信息、执行指令等功能。

通过智能助手可以调节系统音量、屏幕亮度、还可以开启/关闭电源节能模式、WiFi或蓝牙。
...
![1|launch](fig/h_fullscreen.jpg)![1|launch](fig/h_mini.jpg)
...
登录Union ID后，您可以使用云同步、应用商店、邮件客户端、浏览器等应用上相关云服务功能。

首次登录时，会弹出 **隐私政策** 窗口，如果用户需要开启云服务等功能，可以勾选 **我已阅读并同意《隐私政策》**，单击 **确定** 开启云服务等功能。

开启云同步功能可自动同步各种系统配置到云端，如网络、声音、鼠标、更新、任务栏、启动器、壁纸、主题、电源等。若想在另一台电脑上使用相同的系统配置，只需登录此网络帐户，即可一键同步以上配置到该设备。
...
当显示屏为单屏时，可以修改屏幕亮度、缩放、分辨率、刷新率和显示方向。
...
1. 在控制中心首页，单击 ![display_normal](../common/display_normal.svg)。
2. 单击  **旋转屏幕** 按钮 ![rotate](../common/rotate.svg) 。
...
1. 在控制中心首页，单击 ![default_applications_normal](../common/default_applications_normal.svg)。
2. 选择文件类型进入默认程序列表。
3. 单击列表下的 的 **添加默认程序** 按钮![add](../common/add.svg)，选择desktop文件（一般在/usr/share/applications)，或特定的二进制文件。
...
![p0|personalise](fig/personalise.png)
...
> ![notes](../common/notes.svg)说明：您还可以打开 **自动同步配置**。
...
> ![tips](../common/tips.svg)窍门：如果不需要检查更新，不需要收到系统或应用的更新提示，可在“更新设置”中关闭“检查更新”或“更新提醒”。

![0|update](fig/h_update.png)
...
由于某些情况，系统没有激活成功，激活状态显示为未激活。
> ![notes](../common/notes.svg)说明：如果系统未激活，授权管理图标 ![authorize1](../common/authorize1.svg)一直常驻在右下角托盘。
...
若网络设置正常，激活服务器连接畅通，用户可以执行以下操作，手动激活授权管理功能。

1. 进入授权管理界面。
   - 点击右下角托盘上的 ![authorize1](../common/authorize1.svg)进入授权管理界面。
   - 打开 **控制中心**，选择 **系统信息**，点击 **关于本机**。可查看版本授权栏，点击 **激活** 进入授权管理界面。
2. 在授权管理界面，点击 **立即激活**，即可激活授权管理功能。

![0|activation_code3](fig/h_activation_code1.png)

#### 已激活

开机自动激活系统，激活成功后，激活状态显示为已激活。可通过控制中心，进入关于本机的系统信息界面查看授权管理信息。

> ![notes](../common/notes.svg)说明：如果系统激活成功，桌面右下角托盘不再显示授权管理图标。

![0|activation_code3](fig/h_activation_code3.png)

在授权管理界面，点击 **查看更多增值服务**，可以跳转至服务活动网站，在该网站用户可以订阅增值服务。


#### 激活服务器设置

如果当前激活服务器崩溃了，通过授权管理工具，点击![menu](../common/icon_menu.svg) 主菜单中的设置，可以自定义新的激活服务器。

### 备份还原

为避免因软件缺陷、硬件损毁、人为操作不当、骇客攻击、电脑病毒、自然灾害等因素造成数据缺失或损坏，可以进行应用数据或系统数据的备份还原，以保障系统的正常运行。

![0|backup-recovery](fig/backupRestore.png)
...
> ![attention](../common/attention.svg)注意： 全盘备份无法备份在自己本身的磁盘里，只能备份在其它存储介质中。

3. 单击 **开始备份**，弹出密码输入框，请输入密码授权备份文件，直至文件备份完成。

> ![notes](../common/notes.svg)说明： 在备份文件的过程中，请不要拔掉电源或强行关机，以防止数据的丢失或损坏。
...
在开机时，在选择系统界面，单击 **系统还原**，进入统信UOS系统还原。用户可以选择“恢复出厂设置”读取安装时的初始化备份，也可以选择“自定义恢复”将系统或数据恢复到某一历史备份时间点。
