...
{
    connect(this,
            &UseBatteryModule::requestSetScreenBlackDelayOnBattery,
            m_work,
            &PowerWorker::setScreenBlackDelayOnBattery);
    connect(this,
            &UseBatteryModule::requestSetSleepDelayOnBattery,
            m_work,
            &PowerWorker::setSleepDelayOnBattery);
    connect(this,
            &UseBatteryModule::requestSetAutoLockScreenOnBattery,
            m_work,
            &PowerWorker::setLockScreenDelayOnBattery);

    connect(this,
            &UseBatteryModule::requestSetBatteryPressPowerBtnAction,
            m_work,
            &PowerWorker::setBatteryPressPowerBtnAction);
    connect(this,
            &UseBatteryModule::requestSetBatteryLidClosedAction,
            m_work,
            &PowerWorker::setBatteryLidClosedAction);
    connect(this,
            &UseBatteryModule::requestSetLowPowerNotifyEnable,
            m_work,
            &PowerWorker::setLowPowerNotifyEnable);
    connect(this,
            &UseBatteryModule::requestSetLowPowerNotifyThreshold,
            m_work,
            &PowerWorker::setLowPowerNotifyThreshold);
    connect(this,
            &UseBatteryModule::requestSetLowPowerAutoSleepThreshold,
            m_work,
            &PowerWorker::setLowPowerAutoSleepThreshold);
...
void UseBatteryModule::initUI()
{
    ...
    group->appendChild(new ItemModule(
            ...
            [this, delayToLiteralString](ModuleObject *module) -> QWidget * {
                TitledSliderItem *monitorSleepOnBattery =
                ...
                auto setScreenBlackDelayOnBattery = [monitorSleepOnBattery,
                ...
                connect(m_model,
                        &PowerModel::screenBlackDelayChangedOnBattery,
                        monitorSleepOnBattery,
                        setScreenBlackDelayOnBattery);
                setScreenBlackDelayOnBattery(m_model->screenBlackDelayOnBattery());
                connect(monitorSleepOnBattery->slider(),
    ...
    group->appendChild(new ItemModule(
            ...
            [this, delayToLiteralString](ModuleObject *module) -> QWidget * {
                ...
                auto setAutoLockScreenOnBattery = [autoLockScreen,
                                                   &delayToLiteralString](const int delay) {
                ...
                connect(m_model,
                        &PowerModel::batteryLockScreenDelayChanged,
                        autoLockScreen,
                        setAutoLockScreenOnBattery);
    ...
    group->appendChild(new ItemModule(
            ...
            [this, delayToLiteralString](ModuleObject *module) -> QWidget * {
                TitledSliderItem *computerSleepOnBattery =
                ...
                auto setScreenBlackDelayOnBattery = [computerSleepOnBattery,
                                                     &delayToLiteralString](const int delay) {
                ...
                connect(m_model,
                        &PowerModel::sleepDelayChangedOnBattery,
                        computerSleepOnBattery,
                        setScreenBlackDelayOnBattery);
                computerSleepOnBattery->setVisible(m_model->canSuspend() && m_model->getSuspend());
                connect(computerSleepOnBattery->slider(),
                        &DCCSlider::valueChanged,
                        this,
                        &UseBatteryModule::requestSetSleepDelayOnBattery);
            ...
            [this](ModuleObject *module) -> QWidget * {
                ...
                connect(cmbCloseLid,
                        ...
                        [this, cmbCloseLid](int index) {
                            Q_EMIT requestSetBatteryLidClosedAction(
                                    cmbCloseLid->itemData(index).toInt());
                        });
                connect(m_model,
                        &PowerModel::batteryLidClosedActionChanged,
                        cmbCloseLid,
                        setCloseLidData);
                return cmbCloseLid;
            });
    ...
    group->appendChild(
            new ItemModule("whenThePowerButtonIsPressed",
                           ...
                           [this](ModuleObject *module) -> QWidget * {
                               ...
                               auto setPowerButtonData = [this, cmbPowerBtn]() {
                                   ...
                                   for (int i = 0; i < cmbPowerBtn->count(); i++) {
                                       if (cmbPowerBtn->itemData(i).toInt()
                                           == m_model->batteryPressPowerBtnAction()) {
                               ...
                               connect(cmbPowerBtn,
                                       QOverload<int>::of(&AlertComboBox::currentIndexChanged),
                                       this,
                                       [this, cmbPowerBtn](int index) {
                                           Q_EMIT requestSetBatteryPressPowerBtnAction(
                                                   cmbPowerBtn->itemData(index).toInt());
                                       });
                               connect(m_model,
                                       &PowerModel::batteryPressPowerBtnActionChanged,
                                       cmbPowerBtn,
                                       setPowerButtonData);
                               return cmbPowerBtn;
                           }));
    ...
    group->appendChild(new ItemModule(
            ...
            [this](ModuleObject *module) -> QWidget * {
                ...
                connect(m_model,
                        &PowerModel::lowPowerNotifyEnableChanged,
                        swBatteryHint,
                        &DSwitchButton::setChecked);

                connect(swBatteryHint,
    ...
    ItemModule *itemLowBatteryHint = new ItemModule(
            "lowBatteryLevel",
            tr("Low battery level"),
            [this](ModuleObject *module) -> QWidget * {
                ...
                if (bindValues.indexOf(m_model->lowPowerNotifyThreshold())
                    < cmbLowBatteryHint->count()) {
                    cmbLowBatteryHint->setCurrentIndex(
                            bindValues.indexOf(m_model->lowPowerNotifyThreshold()));
                }
                connect(cmbLowBatteryHint,
                        QOverload<int>::of(&DComboBox::currentIndexChanged),
                        this,
                        [=](int index) {
                ...
                connect(m_model,
                        &PowerModel::lowPowerNotifyThresholdChanged,
                        cmbLowBatteryHint,
                        [=](int value) {
    ...
    connect(m_model,
            &PowerModel::lowPowerNotifyEnableChanged,
            itemLowBatteryHint,
            [itemLowBatteryHint, this](/*const bool state*/) {
                itemLowBatteryHint->setHidden(
                        !(m_model->haveBettary() && m_model->lowPowerNotifyEnable()));
            });
    group->appendChild(itemLowBatteryHint);

    ItemModule *itemAutoSuspendBatteryLevel = new ItemModule(
            "autoSuspendBatteryLevel",
            tr("Auto suspend battery level"),
            [this](ModuleObject *module) -> QWidget * {
                ...
                connect(m_model,
                        &PowerModel::lowPowerAutoSleepThresholdChanged,
                        cmbAutoSuspend,
                        [=](const int value) {
    ...
    appendChild(group);
    group->appendChild(new ItemModule(
            "displayRemainingUsingAndChargingTime",
            tr("Display remaining using and charging time"),
            [](ModuleObject *module) -> QWidget * {
                DSwitchButton *powerShowTimeToFull = new DSwitchButton();
                // depend dock dconfig setting "showtimetofull"
                DConfig *cfgDock = DConfig::create("org.deepin.dde.dock1",
                                                   "org.deepin.dde.dock1",
                                                   QString(),
                                                   powerShowTimeToFull);
                connect(powerShowTimeToFull,
                        &DSwitchButton::checkedChanged,
                        powerShowTimeToFull,
                        [cfgDock, powerShowTimeToFull]() {
                            // 保存设置值
                            if (!cfgDock->value("showtimetofull").isNull()) {
                                cfgDock->setValue("showtimetofull",
                                                  powerShowTimeToFull->isChecked());
                            }
                            connect(cfgDock,
                                    &DConfig::valueChanged,
                                    powerShowTimeToFull,
                                    [cfgDock, powerShowTimeToFull](const QString &key) {
                                        if ("showtimetofull" == key) {
                                            powerShowTimeToFull->setChecked(
                                                    cfgDock->value("showtimetofull").toBool());
                                        }
                                    });
                        });
    ...
    group->appendChild(new ItemModule("maximumCapacity",
                                      tr("Maximum capacity"),
                                      [this](ModuleObject *module) -> QWidget * {
                                          TitleValueItem *ShowTimeToFullTips = new TitleValueItem();
                                          ShowTimeToFullTips->setValue(
                                                  QString::number(int(m_work->getBatteryCapacity()))
                                                  + "%");
