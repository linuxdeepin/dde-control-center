...
{
    connect(this,
            &UseElectricModule::requestSetScreenBlackDelayOnPower,
            m_work,
            &PowerWorker::setScreenBlackDelayOnPower);
    connect(this,
            &UseElectricModule::requestSetSleepDelayOnPower,
            m_work,
            &PowerWorker::setSleepDelayOnPower);
    connect(this,
            &UseElectricModule::requestSetSleepOnLidOnPowerClosed,
            m_work,
            &PowerWorker::setSleepOnLidOnPowerClosed); // Suspend on lid close  not using???
    connect(this,
            &UseElectricModule::requestSetAutoLockScreenOnPower,
            m_work,
            &PowerWorker::setLockScreenDelayOnPower);

    connect(this,
            &UseElectricModule::requestSetLinePowerPressPowerBtnAction,
            m_work,
            &PowerWorker::setLinePowerPressPowerBtnAction);
    connect(this,
            &UseElectricModule::requestSetLinePowerLidClosedAction,
            m_work,
            &PowerWorker::setLinePowerLidClosedAction);
...
void UseElectricModule::initUI()
{
    ...
    SettingsGroupModule *group =
            new SettingsGroupModule("screenAndSuspendGroup", tr("Screen and Suspend"));
    group->setSpacing(10);
    appendChild(group);
    QStringList annos;
    annos << "1m"
    ...
    group->appendChild(new ItemModule(
            "turnOffTheMonitorAfter",
            tr("Turn off the monitor after"),
            [this, annos, &delayToLiteralString](ModuleObject *module) -> QWidget * {
                TitledSliderItem *monitorSleepOnPower =
                        new TitledSliderItem(tr("Turn off the monitor after"));
                ...
                connect(monitorSleepOnPower->slider(),
                        &DCCSlider::valueChanged,
                        this,
                        &UseElectricModule::requestSetScreenBlackDelayOnPower);
                auto setScreenBlackDelayOnPower = [monitorSleepOnPower,
                                                   &delayToLiteralString](const int delay) {
                ...
                connect(m_model,
                        &PowerModel::screenBlackDelayChangedOnPower,
                        monitorSleepOnPower,
                        setScreenBlackDelayOnPower);
    ...
    group->appendChild(new ItemModule(
            ...
            [this, annos, &delayToLiteralString](ModuleObject *module) -> QWidget * {
                ...
                connect(m_model,
                        &PowerModel::powerLockScreenDelayChanged,
                        autoLockScreen,
                        setLockScreenAfter);
                connect(autoLockScreen->slider(),
                        &DCCSlider::valueChanged,
                        this,
                        &UseElectricModule::requestSetAutoLockScreenOnPower);
    ...
    if (!IsServerSystem) {
        group->appendChild(new ItemModule(
                "computerSuspendsAfter",
                tr("Computer suspends after"),
                [this, annos, &delayToLiteralString](ModuleObject *module) -> QWidget * {
                    TitledSliderItem *computerSleepOnPower =
                            new TitledSliderItem(tr("Computer suspends after"));
                    ...
                    connect(computerSleepOnPower->slider(),
                            &DCCSlider::valueChanged,
                            this,
                            &UseElectricModule::requestSetSleepDelayOnPower);
                    auto setSleepDelayOnPower = [computerSleepOnPower,
                                                 &delayToLiteralString](const int delay) {
                    ...
                    connect(m_model,
                            &PowerModel::sleepDelayChangedOnPower,
                            computerSleepOnPower,
                            setSleepDelayOnPower);
                    computerSleepOnPower->setVisible(m_model->canSuspend()
                                                     && m_model->getSuspend());
    ...
    // combox
    ItemModule *itemLidIsClosed = new ItemModule(
            "whenTheLidIsClosed",
            tr("When the lid is closed"),
            [this](ModuleObject *module) -> QWidget * {
                ...
                auto setCloseLidData = [this, cmbCloseLid]() {
                    ...
                    for (int i = 0; i < cmbCloseLid->count(); i++) {
                        if (cmbCloseLid->itemData(i).toInt()
                            == m_model->linePowerLidClosedAction()) {
                ...
                connect(m_model,
                        &PowerModel::linePowerLidClosedActionChanged,
                        cmbCloseLid,
                        setCloseLidData);
                connect(cmbCloseLid,
                        QOverload<int>::of(&AlertComboBox::currentIndexChanged),
                        this,
                        [this, cmbCloseLid](int index) {
                            Q_EMIT requestSetLinePowerLidClosedAction(
                                    cmbCloseLid->itemData(index).toInt());
                        });
                return cmbCloseLid;
            });
    ...
    group->appendChild(new ItemModule(
            ...
            tr("When the power button is pressed"),
            [this](ModuleObject *module) -> QWidget * {
                ...
                auto setPowerButtonData = [this, cmbPowerButton]() {
                    ...
                    for (int i = 0; i < cmbPowerButton->count(); i++) {
                        if (cmbPowerButton->itemData(i).toInt()
                            == m_model->linePowerPressPowerBtnAction()) {
                ...
                connect(m_model,
                        &PowerModel::linePowerPressPowerBtnActionChanged,
                        cmbPowerButton,
                        setPowerButtonData);
                connect(cmbPowerButton,
                        QOverload<int>::of(&AlertComboBox::currentIndexChanged),
                        this,
                        [this, cmbPowerButton](int index) {
                            Q_EMIT requestSetLinePowerPressPowerBtnAction(
                                    cmbPowerButton->itemData(index).toInt());
                        });
