...
PersonalizationDBusProxy::PersonalizationDBusProxy(QObject *parent)
    : QObject(parent)
    , m_AppearanceInter(new QDBusInterface(AppearanceService,
                                           AppearancePath,
                                           AppearanceInterface,
                                           QDBusConnection::sessionBus(),
                                           this))
    , m_WMSwitcherInter(new QDBusInterface(WMSwitcherService,
                                           WMSwitcherPath,
                                           WMSwitcherInterface,
                                           QDBusConnection::sessionBus(),
                                           this))
    , m_WMInter(new QDBusInterface(
              WMService, WMPath, WMInterface, QDBusConnection::sessionBus(), this))
    , m_EffectsInter(new QDBusInterface(
              EffectsService, EffectsPath, EffectsInterface, QDBusConnection::sessionBus(), this))

{
    ...
    QDBusConnection::sessionBus().connect(WMService,
                                          WMPath,
                                          PropertiesInterface,
                                          PropertiesChanged,
                                          this,
    ...
    connect(m_AppearanceInter,
            SIGNAL(Changed(const QString &, const QString &)),
            this,
            SIGNAL(Changed(const QString &, const QString &)));
    connect(m_AppearanceInter,
            SIGNAL(Refreshed(const QString &)),
            this,
            SIGNAL(Refreshed(const QString &)));
    connect(m_WMSwitcherInter,
            SIGNAL(WMChanged(const QString &)),
            this,
            SIGNAL(WMChanged(const QString &)));
    connect(m_WMInter,
            SIGNAL(compositingEnabledChanged(bool)),
            this,
            SIGNAL(compositingEnabledChanged(bool)));
}
...
QString PersonalizationDBusProxy::List(const QString &ty)
{
    return QDBusPendingReply<QString>(
            m_AppearanceInter->asyncCall(QStringLiteral("List"), QVariant::fromValue(ty)));
}

bool PersonalizationDBusProxy::List(const QString &ty,
                                    QObject *receiver,
                                    const char *member,
                                    const char *errorSlot)
{
    ...
    return m_AppearanceInter->callWithCallback(QStringLiteral("List"),
                                               args,
                                               receiver,
                                               member,
                                               errorSlot);
...
{
    m_AppearanceInter->asyncCall(QStringLiteral("Set"),
                                 QVariant::fromValue(ty),
                                 QVariant::fromValue(value));
}

QString PersonalizationDBusProxy::Show(const QString &ty, const QStringList &names)
{
    return QDBusPendingReply<QString>(m_AppearanceInter->asyncCall(QStringLiteral("Show"),
                                                                   QVariant::fromValue(ty),
                                                                   QVariant::fromValue(names)));
}

bool PersonalizationDBusProxy::Show(const QString &ty,
...
{
    return QDBusPendingReply<QString>(m_AppearanceInter->asyncCall(QStringLiteral("Thumbnail"),
                                                                   QVariant::fromValue(ty),
                                                                   QVariant::fromValue(name)));
}

bool PersonalizationDBusProxy::Thumbnail(const QString &ty,
                                         const QString &name,
                                         QObject *receiver,
                                         const char *member,
                                         const char *errorSlot)
{
    ...
    return m_AppearanceInter->callWithCallback(QStringLiteral("Thumbnail"),
                                               args,
                                               receiver,
                                               member,
                                               errorSlot);
...
bool PersonalizationDBusProxy::loadEffect(const QString &name)
{
    return QDBusPendingReply<bool>(
            m_EffectsInter->asyncCall(QStringLiteral("loadEffect"), QVariant::fromValue(name)));
}
...
{
    return QDBusPendingReply<bool>(
            m_EffectsInter->asyncCall(QStringLiteral("isEffectLoaded"), QVariant::fromValue(name)));
}
                                              ...
                                              const char *member)
{
    ...
    return m_EffectsInter->callWithCallback(QStringLiteral("isEffectLoaded"),
                                            args,
                                            receiver,
                                            member);
}
