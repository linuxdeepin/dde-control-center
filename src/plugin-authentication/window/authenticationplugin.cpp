...
ModuleObject *AuthenticationPlugin::module()
{
    ...
    // 二级 -- 指纹
    ModuleObject *moduleFinger =
            new PageModule("fingerprint", tr("Fingerprint"), authenticationInterface);
    FingerModule *fingerPage =
            new FingerModule(authenticationInterface->model(), authenticationInterface->work());
    ...
    connect(authenticationInterface->model(),
            &CharaMangerModel::vaildFingerChanged,
            moduleFinger,
            [moduleFinger](const bool isVaild) {
    ...
    FaceModule *facePage =
            new FaceModule(authenticationInterface->model(), authenticationInterface->work());
    ...
    IrisModule *irisPage =
            new IrisModule(authenticationInterface->model(), authenticationInterface->work());
...
AuthenticationModule::AuthenticationModule(QObject *parent)
    : HListModule("authentication",
                  tr("Biometric Authentication"),
                  tr("Biometric Authentication"),
                  QIcon::fromTheme("dcc_nav_authentication"),
                  parent)
...
QWidget *FingerModule::page()
{
    ...
    connect(w,
            &FingerDetailWidget::requestStopEnroll,
            m_worker,
            &CharaMangerWorker::stopFingerEnroll);
    connect(w,
            &FingerDetailWidget::requestDeleteFingerItem,
            m_worker,
            &CharaMangerWorker::deleteFingerItem);
    connect(w,
            &FingerDetailWidget::requestRenameFingerItem,
            m_worker,
            &CharaMangerWorker::renameFingerItem);
    connect(w,
            &FingerDetailWidget::noticeEnrollCompleted,
            m_worker,
            &CharaMangerWorker::refreshFingerEnrollList);
    return w;
}
...
QWidget *FaceModule::page()
{
    ...
    connect(w,
            ...
            m_worker,
            &CharaMangerWorker::deleteCharaItem);
    connect(w,
            &FaceidDetailWidget::requestRenameFaceItem,
            m_worker,
            &CharaMangerWorker::renameCharaItem);
    connect(w,
