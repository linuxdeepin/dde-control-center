...
ModuleObject *KeyboardPlugin::module()
{
    ...
    // 为二级菜单-键盘添加children
    GeneralSettingModule *generalSettingModule =
            new GeneralSettingModule(moduleInterface->model(), moduleInterface->worker());
    ...
    // 键盘布局
    KBLayoutSettingModule *kBLayoutSettingModule =
            new KBLayoutSettingModule(moduleInterface->model(), moduleInterface->worker());
    ...
    SystemLanguageSettingModule *systemLanguageSettingModule =
            new SystemLanguageSettingModule(moduleInterface->model(), moduleInterface->worker());
    ...
    // 二级菜单--快捷键
    ShortCutSettingMenuModule *moduleShortCutSetting =
            new ShortCutSettingMenuModule("keyboardShortcuts", tr("Shortcuts"));
    ShortCutSettingModule *shortCutSettingModule =
            new ShortCutSettingModule(moduleInterface->model(),
                                      moduleInterface->worker(),
                                      moduleInterface->shortcutModel());
    moduleShortCutSetting->appendChild(shortCutSettingModule);
    ModuleObject *customShortcutModule = new WidgetModule<KeyboardFloatingButton>(
            "AddCustomShortCut",
            "AddCustomShortCut",
            [shortCutSettingModule](KeyboardFloatingButton *customShortcut) {
                customShortcut->setIcon(DStyle::SP_IncreaseElement);
                customShortcut->setObjectName("AddCustomShortCut");
                connect(customShortcut,
                        &DFloatingButton::clicked,
                        shortCutSettingModule,
                        &ShortCutSettingModule::onPushCustomShortcut);
            });
...
void KBLayoutSettingModule::onPushKeyboard(const QStringList &kblist)
{
    ...
    connect(m_worker,
            &KeyboardWorker::onLettersChanged,
            kbLayoutDialog,
            &KeyboardLayoutDialog::setLetters);

    dataControll(m_worker->getDatas());
    kbLayoutDialog->setLetters(m_worker->getLetters());

    connect(kbLayoutDialog,
            &KeyboardLayoutDialog::layoutSelected,
            m_worker,
            &KeyboardWorker::addUserLayout);
...
QWidget *KBLayoutSettingModule::page()
{
    ...
    connect(w,
            &KBLayoutSettingWidget::requestCurLayoutAdded,
            this,
            &KBLayoutSettingModule::setCurrentLayout);
...
QWidget *SystemLanguageSettingModule::page()
{
    ...
    connect(w,
            ...
            this,
            &SystemLanguageSettingModule::onPushSystemLanguageSetting);
...
void SystemLanguageSettingModule::onPushSystemLanguageSetting()
{
    ...
    connect(systemLanguageSettingDialog,
            ...
            this,
            &SystemLanguageSettingModule::onAddLocale);
...
QWidget *ShortCutSettingModule::page()
{
    ...
    connect(w,
            &ShortCutSettingWidget::requestShowConflict,
            this,
            &ShortCutSettingModule::onPushConflict);
    connect(w,
            &ShortCutSettingWidget::requestSaveShortcut,
            m_worker,
            &KeyboardWorker::modifyShortcutEdit);
    connect(w,
            &ShortCutSettingWidget::requestDisableShortcut,
            m_worker,
            &KeyboardWorker::onDisableShortcut);
    connect(w,
            &ShortCutSettingWidget::shortcutEditChanged,
            this,
            &ShortCutSettingModule::onShortcutEdit);
...
{
    CustomContentDialog *content =
            new CustomContentDialog(m_shortcutModel, qobject_cast<QWidget *>(sender()));
    connect(content, &CustomContentDialog::requestUpdateKey, m_worker, &KeyboardWorker::updateKey);
    connect(content,
            &CustomContentDialog::requestAddKey,
            m_worker,
            &KeyboardWorker::addCustomShortcut);
    connect(content,
            &CustomContentDialog::requestForceSubs,
            m_worker,
            &KeyboardWorker::onDisableShortcut);
...
void ShortCutSettingModule::onPushConflict(ShortcutInfo *info, const QString &shortcut)
{
    ...
    connect(scContentDialog,
            &ShortcutContentDialog::requestSaveShortcut,
            m_worker,
            &KeyboardWorker::modifyShortcutEdit);
    connect(scContentDialog,
            &ShortcutContentDialog::requestUpdateKey,
            m_worker,
            &KeyboardWorker::updateKey);
    connect(scContentDialog,
            &ShortcutContentDialog::requestDisableShortcut,
            m_worker,
            &KeyboardWorker::onDisableShortcut);
...
void ShortCutSettingModule::onShortcutEdit(ShortcutInfo *info)
{
    ...
    connect(customEdit,
            &CustomEdit::requestSaveShortcut,
            m_worker,
            &KeyboardWorker::modifyCustomShortcut);
