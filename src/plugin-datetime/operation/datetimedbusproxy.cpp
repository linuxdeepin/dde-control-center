...
DatetimeDBusProxy::DatetimeDBusProxy(QObject *parent)
    : QObject(parent)
    , m_timedateInter(new QDBusInterface(TimedateService,
                                         TimedatePath,
                                         TimedateInterface,
                                         QDBusConnection::sessionBus(),
                                         this))
    , m_systemtimedatedInter(new QDBusInterface(SystemTimedatedService,
                                                SystemTimedatedPath,
                                                SystemTimedatedInterface,
                                                QDBusConnection::systemBus(),
                                                this))
{
    registerZoneInfoMetaType();
    QDBusConnection::sessionBus().connect(TimedateService,
                                          TimedatePath,
                                          PropertiesInterface,
                                          PropertiesChanged,
                                          this,
                               ...
                               const char *errorSlot)
{
    ...
    m_timedateInter->callWithCallback(QStringLiteral("SetNTP"),
                                      argumentList,
                                      receiver,
                                      member,
                                      errorSlot);
...
void DatetimeDBusProxy::SetDate(const QDateTime &datetime, QObject *receiver, const char *member)
{
    ...
    QList<QVariant> argumentList;
    argumentList << QVariant::fromValue(date.year()) << QVariant::fromValue(date.month())
                 << QVariant::fromValue(date.day());
    argumentList << QVariant::fromValue(time.hour()) << QVariant::fromValue(time.minute())
                 << QVariant::fromValue(time.second()) << 0;
...
QStringList DatetimeDBusProxy::GetSampleNTPServers()
{
    return QDBusPendingReply<QStringList>(
            m_timedateInter->asyncCall(QStringLiteral("GetSampleNTPServers")));
}

bool DatetimeDBusProxy::GetSampleNTPServers(QObject *receiver, const char *member)
{
    QList<QVariant> argumentList;
    return m_timedateInter->callWithCallback(QStringLiteral("GetSampleNTPServers"),
                                             argumentList,
                                             receiver,
                                             member);
}
...
{
    return QDBusPendingReply<ZoneInfo>(
            m_timedateInter->asyncCall(QStringLiteral("GetZoneInfo"), zone));
}
...
bool DatetimeDBusProxy::GetZoneInfo(const QString &zone, QObject *receiver, const char *member)
{
    ...
    return m_timedateInter->callWithCallback(QStringLiteral("GetZoneInfo"),
                                             argumentList,
                                             receiver,
                                             member);
}
...
void DatetimeDBusProxy::SetNTPServer(const QString &server,
                                     ...
                                     const char *errorSlot)
{
    ...
    m_systemtimedatedInter->callWithCallback(QStringLiteral("SetNTPServer"),
                                             argumentList,
                                             receiver,
                                             member,
                                             errorSlot);
...
{
    QDBusInterface dbus("org.deepin.dde.LangSelector1",
                        "/org/deepin/dde/LangSelector1",
                        "org.deepin.dde.LangSelector1",
                        QDBusConnection::sessionBus());
