...
FormatSettingModule::FormatSettingModule(DatetimeModel *model,
                                         DatetimeWorker *work,
                                         QObject *parent)
                           ...
                           tr("Sunday") })
{
    ...
    appendChild(new WidgetModule<SwitchWidget>("24hourTime",
                                               tr("24-hour Time"),
                                               [this](SwitchWidget *hourTypeSwitch) {
                                                   hourTypeSwitch->setTitle(tr("24-hour Time"));
                                                   hourTypeSwitch->setChecked(
                                                           m_model->get24HourFormat());
                                                   hourTypeSwitch->addBackground();
                                                   // true : 24 hour type  ,  false : 12 hour type
                                                   connect(hourTypeSwitch,
                                                           &SwitchWidget::checkedChanged,
                                                           m_work,
                                                           &DatetimeWorker::set24HourType);
                                                   connect(m_model,
                                                           &DatetimeModel::hourTypeChanged,
                                                           hourTypeSwitch,
                                                           &SwitchWidget::setChecked);
                                               }));

    appendChild(new WidgetModule<SettingsGroup>("TimeFormat",
                                                tr("Time Format"),
                                                this,
                                                &FormatSettingModule::initTimeFotmat));
}
